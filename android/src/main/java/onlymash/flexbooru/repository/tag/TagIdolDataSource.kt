 ...
 */
class TagIdolDataSource(
    private val idolApi: IdolApi,
    private val search: SearchTag,
    retryExecutor: Executor
) : BasePageKeyedDataSource<Int, TagIdol>(retryExecutor) {

    override fun loadInitialRequest(
        params: LoadInitialParams<Int>,
        callback: LoadInitialCallback<Int, TagIdol>
    ) {
        val request = idolApi.getTags(IdolUrlHelper.getTagUrl(search = search, page = 1))
    ...
    override fun loadAfterRequest(params: LoadParams<Int>, callback: LoadCallback<Int, TagIdol>) {
        val page = params.key
        idolApi.getTags(IdolUrlHelper.getTagUrl(search = search, page = page))
            .enqueue(object : retrofit2.Callback<MutableList<TagIdol>> {
                override fun onFailure(call: Call<MutableList<TagIdol>>, t: Throwable) {
                    loadAfterOnFailed(t.message ?: "unknown err", params, callback)
                }

                override fun onResponse(
                    call: Call<MutableList<TagIdol>>,
                    response: Response<MutableList<TagIdol>>
                ) {
